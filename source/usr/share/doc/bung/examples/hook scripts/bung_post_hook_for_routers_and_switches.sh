...
# Purpose
#   * A post-hook script for use with bung
#   * Check age of last backup:
#       * If less than max age, message and return $post_hook_rc_i
#       * Else message and return $post_hook_rc_e
#
...
function initialise {
    ...
    readonly my_name=${0##*/}
    readonly post_hook_rc_e=2
    readonly post_hook_rc_i=0
    readonly post_hook_rc_w=1
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    [[ ! $opt_f_flag ]] && emsg+=$msg_lf'-f option is required'
    ...
    if [[ $opt_f_flag && ! ${fqdn:-} =~ $fqdn_OK_regex ]]; then
        emsg+=$msg_lf"Invalid router FQDN '$fqdn'"
    ...
    [[ $buf != '' ]] && msg E "$buf"
    max_age=${array[2]}
    ck_uint "$max_age" || msg E "max_age is not an unsigned integer ($max_age)"
...
function msg {
    ...
    if [[ $class = I ]]; then
        ...
        if [[ $class = E ]]; then
            ...
            [[ ! $finalising_flag ]] && finalise $post_hook_rc_e
        fi
    fi  
